...
from grank import npi_rank_v


class TestRankData:
    def test_empty(self):
        """stats.npi_rank_v([]) should return an empty array."""
    ...
    def test_one(self):
        """Check stats.npi_rank_v with an array of length 1."""
        data = [100]
    ...
    def test_basic(self):
        """Basic tests of stats.npi_rank_v."""
        data = [100, 10, 50]
    ...
    def test_rankdata_object_string(self):
        ...
        def check_ranks(a):
            for method in "min", "max", "dense", "ordinal", "average":
                out = npi_rank_v(a, method=method)
    ...
    def test_size_0_axis(self, axis, method, dtype):
        shape = (3, 0)
        data = np.zeros(shape)
        r = npi_rank_v(data, method=method, axis=axis)
...
def test_cases():
    for values, method, expected in _cases:
        ...
        r = npi_rank_v(values, group, method=method)
        assert_array_equal(r, expected)
...
def test_manual():
    gid = [0, 1, 0, 1, 0, 1]
    val = [1, 3, 6, 8, 4, 3]
    ranks = npi_rank_v(val, gid, method="average")
    assert_array_equal(ranks, [1, 1.5, 3, 3, 2, 1.5])

    ranks = npi_rank_v(val, gid, method="min")
    assert_array_equal(ranks, [1, 1, 3, 3, 2, 1])

    ranks = npi_rank_v(val, gid, method="max")
    assert_array_equal(ranks, [1, 2, 3, 3, 2, 2])

    ranks = npi_rank_v(val, gid, method="ordinal")
    assert_array_equal(ranks, [1, 1, 3, 3, 2, 2])

    ranks = npi_rank_v(val, gid, method="dense")
